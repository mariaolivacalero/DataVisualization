...
def main():
    ...
    if len(tracks_without_features):
        print('Connecting to Spotify to extract features and album data...')
        acquired = 0
        track_ids_to_process = [idd for track, idd in track_ids.items() if idd is not None and track in tracks_without_features]
        track_features = {}
        

    # Split track_ids into batches of 50
    for i in range(0, len(track_ids_to_process), 50):
        batch_ids = track_ids_to_process[i:i + 50]
        try:
            track_features= history.get_api_features_and_album(batch_ids,token, track_features)
            acquired += 1
            print(f"Acquired features. Total: {acquired}")
        except Exception as e:
            print(f"Could not retrieve data in batch {i}: {e}")
    ...
    df_final.to_csv('MyData/history.csv')
