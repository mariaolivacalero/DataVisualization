...
def get_api_features_and_album(track_ids: str, token: str, track_features:dict) -> dict:
    sp = spotipy.Spotify(auth=token)
    batch_features = []
    album_data_list = []
    ...
    try:
        batch_album_data = sp.tracks(track_ids)
        for track in batch_album_data['tracks']:
            track_features[track['id']]['album_id'] = track['album']['id']
            track_features[track['id']]['album_name'] = track['album']['name']
    except:
        # Handle errors or retries as needed
        pass
